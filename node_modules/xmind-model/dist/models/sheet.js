...
const langs_1 = require("../common/utils/langs");
const sheet_component_factory_1 = require("./sheet-component-factory");
...
class Sheet extends style_component_1.default {
    constructor(data, options = {}) {
        super(data, options);
        this.componentType = constants_1.MODEL_TYPE.SHEET;
        this._config = new config_1.Config();
        this._idMap = {};
        this._textTranslator = null;
        this.modelEvents = {
    ...
    _initLegend() {
        const legendData = this._data.commit(GET, LEGEND) || {};
        this._legend = this.createComponent(constants_1.MODEL_TYPE.LEGEND, legendData);
        this._legend.parent(this);
    }
    _initRootTopic() {
        this._rootTopic = this.createComponent(constants_1.COMPONENT_TYPE.TOPIC, this._data.commit('get', ROOT_TOPIC_DATA_KEY), {
            type: constants_1.TOPIC_TYPE.ROOT
        });
    ...
    _initRelationships() {
        ...
        relationshipsData && relationshipsData.forEach(data => {
            const relationship = this.createComponent(constants_1.COMPONENT_TYPE.RELATIONSHIP, data);
    ...
    replaceRootTopic(newRootTopicData) {
        this._rootTopic.remove();
        this._data.commit("set", ROOT_TOPIC_DATA_KEY, newRootTopicData);
        this._rootTopic = this.createComponent(constants_1.COMPONENT_TYPE.TOPIC, newRootTopicData, {
            type: constants_1.TOPIC_TYPE.ROOT
        });
        this._rootTopic.parent(this);
    }
    ...
    addRelationship(relationshipData, options = {}) {
        const newRelationship = this.createComponent(constants_1.COMPONENT_TYPE.RELATIONSHIP, relationshipData);
        ...
        this.emit(constants_1.EVENTS.AFTER_SHEET_CONTENT_CHANGE);
    ...
    _initTheme(themeData) {
        if (themeData) {
            const theme = this.createComponent(constants_1.COMPONENT_TYPE.THEME, themeData);
            theme.parent(this);
            this._theme = theme;
        }
    }
    changeTheme(themeData, options = {}) {
        ...
        this.emit(constants_1.EVENTS.AFTER_THEME_CHANGED);
        this.emit(constants_1.EVENTS.AFTER_SHEET_CONTENT_CHANGE);
    }
    changeFreePosition(isFree) {
    ...
    _enableOldFreePosition() {
        if (this._data.commit(GET, TOPIC_POSITIONING) !== undefined)
            return;
        const mainTopics = this.getRootTopic().getChildrenByType(constants_1.TOPIC_TYPE.ATTACHED);
    ...
    changeOverlap(topicOverlapping) {
    ...
    createComponent(componentType, data, options = {}) {
        let model = sheet_component_factory_1.default(this, componentType, data, options);
    ...
    setTextTranslator(fn) {
        this._textTranslator = fn;
    }
    getTranslatedText(key) {
        if (!this._textTranslator) {
            return langs_1.default.translate(constants_1.LANGS.ZH_CN, key);
        }
        else {
