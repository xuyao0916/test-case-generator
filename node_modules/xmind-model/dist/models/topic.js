...
const defaultInitOptions = Object.freeze({
    type: constants_1.TOPIC_TYPE.ATTACHED
});
...
class Topic extends style_component_1.default {
    constructor(data, options = utils_1.deepClone(defaultInitOptions)) {
        super(data, options);
        this.componentType = constants_1.COMPONENT_TYPE.TOPIC;
        this.modelEvents = {
            addTopic: 'addTopic',
            ...
            extensionEventMap: {
                add: {
                    [constants_1.EXTENSION_PROVIDER.AUDIO_NOTES]: 'addAudioNotes',
                    [constants_1.EXTENSION_PROVIDER.TASK_INFO]: 'addTaskInfo',
                    [constants_1.EXTENSION_PROVIDER.UNBALANCED_MAP]: 'addUnbalancedMapInfo'
                },
                remove: {
                    [constants_1.EXTENSION_PROVIDER.AUDIO_NOTES]: 'removeAudioNotes',
                    [constants_1.EXTENSION_PROVIDER.TASK_INFO]: 'removeTaskInfo',
                    [constants_1.EXTENSION_PROVIDER.UNBALANCED_MAP]: 'removeUnbalancedMapInfo'
                }
            },
            labelsChanged: 'labelsChanged',
            informationChanged: 'informationChanged',
        };
        this._childrenTopicMap = {};
        this._boundaries = [];
        this._summaries = [];
        this._markers = [];
        this._image = null;
        ...
        this._extensionMap = {};
        this._topicType = options.type || constants_1.TOPIC_TYPE.ATTACHED;
    }
    init(sheet) {
        super.init(sheet);
        // markers
    ...
    _initChildren() {
        ...
        Object.keys(childrenData).forEach(topicType => {
            const childrenDataList = childrenData[topicType];
    ...
    addChildTopic(childTopicData, options = defaultInitOptions) {
        ...
        if (utils_1.isUndefined(opts.index) || utils_1.isNull(opts.index) || opts.index < 0) {
    ...
    _addChildTopic(childTopicData, options) {
        const newTopicModel = this.ownerSheet().createComponent(constants_1.COMPONENT_TYPE.TOPIC, childTopicData, options);
        // set index value
        if (utils_1.isUndefined(options.index) || utils_1.isNull(options.index) || options.index < 0) {
            options.index = this.getChildrenByType(options.type).length;
        }
        // put child topic model in _childrenTopicMap
        if (!this._childrenTopicMap[options.type])
            this._childrenTopicMap[options.type] = [];
        this._childrenTopicMap[options.type].splice(options.index, 0, newTopicModel);
        // set parent
        newTopicModel.parent(this);
        return newTopicModel;
    }
    getChildIndexByType(topicType, topic) {
    ...
    removeChildTopic(childTopic) {
        ...
        if (!childrenData[childTopicType].length)
            delete childrenData[childTopicType];
        // 若children中已不存在任何内容
        if (utils_1.isEmpty(childrenData))
            this._data.commit('delete', CHILDREN_DATA_KEY);
        childTopic.remove();
    ...
    moveChildTopic(from, to) {
        if (utils_1.isUndefined(from) || utils_1.isUndefined(to))
            return;
        if (from === to)
            return;
        const childrenList = this._childrenTopicMap[constants_1.TOPIC_TYPE.ATTACHED];
        if (!Array.isArray(childrenList))
            return;
        ...
        this._childrenTopicMap[constants_1.TOPIC_TYPE.ATTACHED].splice(from, 1);
        this._childrenTopicMap[constants_1.TOPIC_TYPE.ATTACHED].splice(to, 0, topicModelToMove);
        // move json
        const childrenData = this._data.commit('get', CHILDREN_DATA_KEY);
        childrenData[constants_1.TOPIC_TYPE.ATTACHED].splice(from, 1);
        childrenData[constants_1.TOPIC_TYPE.ATTACHED].splice(to, 0, topicModelToMove.toJSON());
    ...
    addBoundary(boundaryData, options = {}) {
    ...
    _addBoundary(boundaryData, options = {}) {
        const newBoundaryModel = this.ownerSheet().createComponent(constants_1.COMPONENT_TYPE.BOUNDARY, boundaryData, options);
    ...
    removeBoundary(boundary) {
        ...
        if (boundariesData.length === 0)
            this._data.commit('delete', BOUNDARIES_DATA_KEY);
        // todo 需要实现
        boundary.remove();
        // todo 参数设计
    ...
    addSummary(summaryData, summaryTopicData, options = {}) {
        ...
        this.addChildTopic(summaryTopicData, { type: constants_1.TOPIC_TYPE.SUMMARY });
    }
    _addSummary(summaryData, options = {}, isInit = false) {
        const newSummaryModel = this.ownerSheet().createComponent(constants_1.COMPONENT_TYPE.SUMMARY, summaryData, options);
        // todo 判断是否已有相同range的summary
        // todo 是否需要实现summary的checkRange方法？
        // update summaries list
        if (options.index)
            options.index = this._summaries.length;
    ...
    _removeSummary(summary) {
        ...
        if (summariesData.length === 0)
            this._data.commit('delete', SUMMARIES_DATA_KEY);
        // todo 需要实现
        summary.remove();
    ...
    _canCollapse() {
        const hasChildren = this.getChildrenByType(constants_1.TOPIC_TYPE.ATTACHED).length > 0;
    ...
    _addMarker(markerData, options = {}) {
        const newMarkerModel = this.ownerSheet().createComponent(constants_1.COMPONENT_TYPE.MARKER, markerData, options);
        // update markers list
    ...
    removeMarker(markerData) {
        ...
        if (markersData.length === 0)
            this._data.commit('delete', MARKERS_DATA_KEY);
        marker.remove();
    ...
    _addImage(imageData, options = {}) {
        const newImageModel = this.ownerSheet().createComponent(constants_1.COMPONENT_TYPE.IMAGE, imageData, options);
    ...
    _addNotes(notesData, options = {}) {
        const newNotesModel = this.ownerSheet().createComponent(constants_1.COMPONENT_TYPE.NOTE, notesData, options);
        // update _notes
    ...
    addNumbering(numberingData, options = {}) {
        // update data
        if (!numberingData) {
    ...
    _addNumbering(numberingData, options = {}) {
        const newNumberingModel = this.ownerSheet().createComponent(constants_1.COMPONENT_TYPE.NUMBERING, numberingData, options);
        // update _numbering
        this._numbering = newNumberingModel;
        return newNumberingModel;
    }
    changeNumbering(key, value) {
        this._numbering.changeNumbering(key, value);
    }
    removeNumbering() {
        const oldNumberingModel = this._numbering;
        if (!oldNumberingModel)
            return;
        // remove _numbering
        this._numbering = null;
        // remove data
    ...
    isRootTopic() {
        return this.getType() === constants_1.TOPIC_TYPE.ROOT;
    }
    getTitle() {
    ...
    changePosition(position) {
        ...
        if (!position || !utils_1.isPointLike(position)) {
            this._data.commit("delete", POSITION_DATA_KEY);
        }
        else {
            if (utils_1.isEqualPoint(position, oldPosition))
                return;
    ...
    changeStructureClass(structureClass) {
        const oldStructureClass = this.getStructureClass();
        if (oldStructureClass === structureClass)
            return;
        if (structureClass) {
    ...
    changeCustomWidth(customWidth) {
        const oldWidth = this.getCustomWidth();
        if (oldWidth === customWidth)
            return;
        if (customWidth) {
    ...
    addExtension(provider, extensionData, options = {}) {
    ...
    _addExtension(provider, extensionData, options = {}) {
        const newExtensionModel = this.ownerSheet().createComponent(constants_1.COMPONENT_TYPE.EXTENSION, extensionData, options);
        // update _extensionMap
        this._extensionMap[provider] = newExtensionModel;
        return newExtensionModel;
    }
    removeExtension(provider) {
    ...
    getAudioNotes() {
        return this._extensionMap[constants_1.EXTENSION_PROVIDER.AUDIO_NOTES];
    }
    addAudioNotes(audioNotesData, options = {}) {
        this.addExtension(constants_1.EXTENSION_PROVIDER.AUDIO_NOTES, audioNotesData, options);
    }
    removeAudioNotes() {
        this.removeExtension(constants_1.EXTENSION_PROVIDER.AUDIO_NOTES);
    }
    getTaskInfo() {
        return this._extensionMap[constants_1.EXTENSION_PROVIDER.TASK_INFO];
    }
    addTaskInfo(taskInfoData, options = {}) {
        this.addExtension(constants_1.EXTENSION_PROVIDER.TASK_INFO, taskInfoData, options);
    }
    removeTaskInfo() {
        this.removeExtension(constants_1.EXTENSION_PROVIDER.TASK_INFO);
    }
    getUnbalancedMapInfo() {
        return this._extensionMap[constants_1.EXTENSION_PROVIDER.UNBALANCED_MAP];
    }
    addUnbalancedMapInfo(unbalancedMapData, options = {}) {
        this.addExtension(constants_1.EXTENSION_PROVIDER.UNBALANCED_MAP, unbalancedMapData, options);
    }
    removeUnbalancedMapInfo() {
        this.removeExtension(constants_1.EXTENSION_PROVIDER.UNBALANCED_MAP);
    }
    remove() {
        super.remove();
        this.parent(null);
